...
def get_compare_key(c: Tuple[str, str, str, str, str]) -> List[Tuple[str, str, int, int]]:
    res = []
...
def print_checks(framework: str = "all", use_bc_ids: bool = False) -> None:
    printable_checks_list = get_checks(framework, use_bc_ids=use_bc_ids)
    print(
        tabulate(
            printable_checks_list, headers=["Id", "Type", "Entity", "Policy", "IaC"], tablefmt="github", showindex=True
        )
    )
    print("\n\n---\n\n")


def get_checks(framework: str = "all", use_bc_ids: bool = False) -> List[Tuple[str, str, str, str, str]]:
